        ...
        for (const name in Game.creeps) {
            ...
            if (creep.memory.role) {
                try {
                    ...
                    console.log(
                        `${creep.name} caught amnesia, former role: ${creep.memory.role}, unsetting role, error: ${err}`,
                    );
    ...
    for (const name in Memory.creeps) {
        ...
        if (creep) {
            if (
                ...
                console.log(
                    creep.name,
                    "is getting old and is finally " + "entitled to some regeneration!",
                );
                creep.memory.role = "regenerate";
            }
        }
...
function workersUnion() {
    ...
    for (const unitRole in Memory.socialStructure) {
        for (let count = 0; count < Memory.population[Memory.socialStructure[unitRole]]; count++) {
    ...
    creepsList = _.sortBy(creepsList, [
        (creep: Creep) => {
