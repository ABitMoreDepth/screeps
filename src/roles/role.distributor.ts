...
export function distributor(creep: Creep) {
    if (
        ...
        !creep.memory.state.toString().match(/^distribute|refill$/)
    ) {
    ...
    if (creep.memory.state === "refill") {
        ...
        if (source !== null) {
            if (creep.withdraw(source as StructureStorage, RESOURCE_ENERGY) === ERR_NOT_IN_RANGE) {
                creep.moveTo(source);
            }
        }
        ...
        if (source !== null) {
            if (
                creep.withdraw(source as StructureContainer, RESOURCE_ENERGY) === ERR_NOT_IN_RANGE
            ) {
                creep.moveTo(source);
            }
            return;
        ...
        source = find_nearest_energy_collection_point(creep);
        if (source !== null) {
            if (
                creep.withdraw(source as StructureContainer, RESOURCE_ENERGY) === ERR_NOT_IN_RANGE
            ) {
